...
labels = [r"$\beta_e$", r"$\beta_{a_1}$", r"$\delta_e$", r"$\delta_m$", r"$\lambda_{\mathrm{Th1},0}$", r"$\xi_\mathrm{IL10}$", r"$\lambda_{e,\mathrm{ag}}$",
...
g = sns.catplot(data = df_relative, x = "param_name", y = "val_norm", col = "Infection",
                hue = "param_val_norm", order = myparams,
                aspect = 1, legend = False,
                kind = "bar", dodge =True,
                palette = ["k", "white"], height = 1.6, color = "white", edgecolor = "k")
g.set_titles(row_template = '{row_name}', col_template = '{col_name}')

g.set_xticklabels(rotation = 90, labels = labels)
g.set(xlabel = "", ylabel = f"Tfh (%all) day {day}", yticks = [-1,0,1])
sns.despine(top = False, right = False)
plt.tight_layout()
plt.show()

g.savefig(f"../../figures/local_sensitivity/sensitivity_barplots_lb_d{day}_acute_vs_chronic.svg")
g.savefig(f"../../figures/local_sensitivity/sensitivity_barplots_lb_d{day}_acute_vs_chronic.pdf")
